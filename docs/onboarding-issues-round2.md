...
**文件**：`onboarding-manager.js` `fetchUserState()` 第 130-139 行

**问题**：`fetchUserState` 中有两处用户切换检测：

1. **第 99-111 行**：在读取缓存时检测用户不匹配 → 清除 `st_onboarding_progress`、`st_celebration_shown`、`st_user_state` ✓ 正确
2. **第 130-139 行**：在 API 返回后检测用户切换 → 只清除 `st_onboarding_progress`，**遗漏了 `st_celebration_shown`** ✗

```javascript
// 第 130-139 行：API 返回后检测用户切换
...
if (cachedStr) {
  try {
    ...
    if (cachedState._userId && cachedState._userId !== data.user.id) {
      ...
      // ← 缺少 localStorage.removeItem('st_celebration_shown');
...
**影响**：用户 A 完成所有任务触发庆典后（`st_celebration_shown` 被设置），在同一浏览器切换到用户 B 登录。用户 B 完成所有任务时，`tryCelebrate` 检查到 `st_celebration_shown` 已存在 → 不弹出庆典。

**修复方案**：在第 135 行后添加 `localStorage.removeItem('st_celebration_shown');`
...
### P1-7：欢迎弹窗"跳过"按钮不调用 `markTourSeen()`，用户每次进首页都看到弹窗

**文件**：`welcome-modal.js` 第 174-178 行

**问题**：上一轮将 `markTourSeen()` 从 `show()` 移到"开始体验引导"按钮的 click 事件中。但"跳过"按钮的 click 事件只调用了 `this.hide()`，没有调用 `markTourSeen()`。
...
**影响**：用户点击"跳过"后，`has_seen_tour` 仍为 false。下次访问首页时 `decideOnboarding()` 场景 1（`!has_seen_tour && currentPage === 'index'`）再次命中，欢迎弹窗反复弹出。

**修复方案**：在"跳过"按钮的 click 事件中也调用 `markTourSeen()`：
...
**影响**：用户点击"创建第一个故事"跳转到 `/create-ai.html?guide=create-ai` 后，`page-guide-bridge.js` 的 `waitAndTrigger()` 函数查找 `guideConfigs['create-ai']` 得到 `undefined`，直接 `return`，不显示任何引导。URL 中的 `?guide=create-ai` 参数虽然会被 `clearGuideParam()` 清除，但用户看不到引导提示。

**修复方案**：两个选择：
- **方案 A**：在 `guideConfigs` 中添加 `create-ai` 配置
...
**文件**：`welcome-modal.js` 第 210-224 行

**问题**：点击遮罩关闭（第 211-213 行）和 ESC 关闭（第 218-223 行）都只调用 `this.hide()`，不调用 `markTourSeen()`。与 P1-7 的"跳过"按钮问题类似，用户通过这些方式关闭弹窗后，`has_seen_tour` 仍为 false，下次进首页仍会弹出。

**修复方案**：统一处理——在 `hide()` 中根据弹窗显示原因决定是否标记，或在所有关闭路径中都调用 `markTourSeen()`。
...
| 类型 | 数量 | 编号 |
|---|---|---|
| 控制流缺陷（缺 return、死分支） | 3 | P0-1, P0-4, P1-5 |
| 错误处理缺陷（API 失败、fallback 错误） | 2 | P0-2, P0-3 |
| 状态不一致（用户切换、缓存过期） | 3 | P0-3, P2-21, P2-25 |
| 重复代码 | 6 | P1-9, P1-11, P2-19, P2-20, P2-23, P2-24 |
| 死代码 | 4 | P1-14, P1-17, P2-22, P3-34 |
| 用户引导体验缺陷 | 6 | P0-4, P1-7, P1-8, P1-10, P1-12, P1-15 |
| 移动端适配 | 1 | P2-30 |
| 逻辑错误（条件判断、边界） | 5 | P1-13, P1-16, P2-27, P2-31, P2-32 |
| 代码健壮性 | 5 | P2-26, P2-29, P2-33, P3-37, P3-41 |
| 数据合并缺陷 | 2 | P2-21, P2-25 |
| 其他 | 4 | P2-18, P2-28, P3-35, P3-36, P3-38, P3-39, P3-40, P3-42 |

### 按文件分类

| 文件 | 问题数 | 主要问题 |
|---|---|---|
| `onboarding-manager.js` | 12 | P0-1, P0-2, P0-3, P1-6, P1-10, P1-17, P2-21, P2-25, P2-29, P3-34, P3-37, P3-40 |
| `tour.js` | 9 | P0-4, P1-5, P1-14, P1-15, P1-16, P2-22, P2-23, P2-24, P2-30, P2-31, P3-35, P3-39 |
| `welcome-modal.js` | 4 | P1-7, P1-8, P2-28, P3-36 |
| `page-guide-bridge.js` | 4 | P1-13, P2-27, P2-33, P3-38 |
| `story-concept-bridge.js` | 3 | P1-9, P1-11, P3-42 |
| `concept-guide.js` | 3 | P1-12, P2-26, P2-32, P3-41 |
...
1. P0-1：添加 return 语句
2. P0-2：修复 API 失败时的 fallback 逻辑
3. P0-3：补充用户切换时清除 `st_celebration_shown`
4. P0-4：create-ai tour 完成后跳转 write 页
5. P1-5：`markTourComplete` 仅在 write 页设置 `writeGuideSeen`
6. P1-15：`getWriteSteps` 最后一步始终设置 `writeGuideSeen`

### 第二批：P1 其余（用户体验修复）
7. P1-6：`decideOnboarding` 使用 `getProgressWithFixup`
8. P1-7：欢迎弹窗"跳过"调用 `markTourSeen`
9. P1-8：修复 `?guide=create-ai` 配置缺失
10. P1-9：统一 `conceptGuide.hide` patch 逻辑
11. P1-10：帮助按钮 allDone 时展示帮助内容
12. P1-11：统一 `markConceptGuideSeen` 实现
13. P1-12：概念引导仅在看完后标记
14. P1-13：添加 `fired` 标志
15. P1-14：删除死代码
16. P1-16：跳过按钮添加存在性检查
17. P1-17：删除 `syncProgressToServer`
...
第一轮计划文档中的 18 个修复项已全部实现。本轮发现的 42 个问题中：

- **4 个 P0** 是第一轮未发现的严重问题
- **P1-5** 是第一轮修复 A1 时引入的回归
- **其余 37 个** 是第一轮分析方法（线索追踪）的盲区
