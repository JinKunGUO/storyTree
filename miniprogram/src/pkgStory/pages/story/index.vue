...
function goChapter(nodeId: number) {
  uni.navigateTo({ url: `/pkgStory/pages/chapter/index?id=${nodeId}` })
}
...
function handleWriteBranch(parentNodeId: number) {
  ...
  uni.navigateTo({
    url: `/pkgWrite/pages/write/editor?storyId=${story.value?.id}&parentId=${parentNodeId}&parentTitle=${encodeURIComponent(parentTitle)}`,
  })
}
...
function goManage() {
  uni.navigateTo({ url: `/pkgStory/pages/story/manage?id=${story.value?.id}` })
}
...
defineExpose({
  onShareAppMessage() {
    return {
      ...
      path: `/pkgStory/pages/story/index?id=${story.value?.id}`,
