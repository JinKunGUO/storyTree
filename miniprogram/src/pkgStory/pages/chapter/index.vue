...
function goChapter(id: number) {
  ...
  uni.redirectTo({ url: `/pkgStory/pages/chapter/index?id=${id}` })
}
...
function writeBranch() {
  uni.navigateTo({
    url: `/pkgWrite/pages/write/editor?storyId=${node.value?.story_id}&parentId=${node.value?.id}&parentTitle=${encodeURIComponent(node.value?.title || '')}`,
  })
}
...
function editChapter() {
  ...
  uni.navigateTo({
    url: `/pkgWrite/pages/write/editor?nodeId=${node.value.id}&storyId=${node.value.story_id}`,
  })
}
...
function handleAiApply(content: string) {
  ...
  uni.navigateTo({
    url: `/pkgWrite/pages/write/editor?storyId=${node.value?.story_id}&parentId=${node.value?.id}&parentTitle=${encodeURIComponent(node.value?.title || '')}&prefillContent=${encodeURIComponent(content || '')}`,
  })
}
...
function goBack() {
  ...
  if (storyPageIdx >= 0) {
    ...
    uni.redirectTo({ url: `/pkgStory/pages/story/index?id=${node.value.story_id}` })
  } else {
...
function goPrevChapter() {
  ...
  uni.redirectTo({ url: `/pkgStory/pages/chapter/index?id=${parent.value.id}` })
}
